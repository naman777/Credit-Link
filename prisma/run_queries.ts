...
// Usage: npx tsx prisma/run_queries.ts
...
async function main() {
  ...
  console.table(repayments.map(r => ({
    ...
    loan_amount: `₹${r.schedule.loan.principal_amount}`,
  ...
  console.table([{
    ...
    total_disbursed: `₹${totalDisbursed._sum.principal_amount || 0}`,
    total_repaid: `₹${totalRepaid._sum.paid_amount || 0}`,
  ...
  console.table(loansByStatus.map(l => ({
    status: l.status,
    count: l._count.id,
    total_amount: `₹${l._sum.requested_amount || 0}`,
  })));
  ...
  console.table(topBorrowers.map(b => {
    ...
    return {
      ...
      total_borrowed: `₹${b._sum.requested_amount || 0}`,
      credit_score: user?.credit_score?.score || 0,
    };
  }).sort((a, b) => parseFloat(b.total_borrowed.replace('₹', '')) - parseFloat(a.total_borrowed.replace('₹', ''))));
  ...
  console.table([{
    ...
    total_disbursed: `₹${loanSummary._sum.principal_amount || 0}`,
    avg_loan: `₹${Math.round(Number(loanSummary._avg.principal_amount) || 0)}`,
  ...
  const auditUserIds = auditLogs.map(a => a.user_id).filter(Boolean) as string[];
  ...
  console.table(lenders.map(l => ({
    ...
    total_invested: `₹${l.loans_as_lender.reduce((sum, loan) => sum + Number(loan.principal_amount), 0)}`,
    active_investments: `₹${l.loans_as_lender.filter(loan => loan.status === 'ACTIVE').reduce((sum, loan) => sum + Number(loan.principal_amount), 0)}`,
    wallet_balance: `₹${l.wallet?.current_balance || 0}`,
  })));
